...
f1=xr.open_dataset('E:/lbm/out1/southchina/linear.t42l21.nc',decode_times=False)
print(f1.time)
lon=np.array(f1.lon)
...
d=Ellipse(xy=(120,25),width=16,height=10,facecolor='blue',linewidth=2,linestyle='-',zorder=0,edgecolor='none',transform=ccrs.PlateCarree())
ax.add_patch(d)
...
f5=xr.open_dataset('E:/lbm/frc1/southchina/frc.t42l21.nc',decode_times=False)
k=np.array(f5.t.loc[0])
t_mean=np.zeros((20))
for i in range(0,20):
    t_mean[i]=np.sum(k[i])/12
t_mean1=t_mean[0:11]*86400

t=np.arange(0,11,1)
font={'family':'Arial'}
totalq_ano_mean=np.array([-0.1586,-0.0990,-0.1274,-0.1027,-0.0970,-0.0859,-0.1850,-0.1340,-0.0259,0.0236,0.1332])
...
plt.savefig('D:/figure改文章12-25/southchinalbm.png',format='png',bbox_inches='tight',dpi=300)
