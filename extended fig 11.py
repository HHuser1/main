...
f1=xr.open_dataset('E:/lbm/out1/northchina/linear.t42l21.nc',decode_times=False)
...
d=Ellipse(xy=(118,38),width=16,height=10,facecolor='r',linewidth=2,linestyle='-',zorder=0,edgecolor='none',transform=ccrs.PlateCarree())
ax.add_patch(d)
...
f5=xr.open_dataset('E:/lbm/frc1/northchina/frc.t42l21.nc',decode_times=False)
print(f5)
k=np.array(f5.t.loc[0])
t_mean=np.zeros((20))
for i in range(0,20):
    t_mean[i]=np.sum(k[i])/15
t_mean1=t_mean[0:11]*86400

t=np.arange(0,11,1)
font={'family':'Arial'}
totalq_ano_mean=np.array([0.0944,0.2461,0.2657,0.2965,0.2368,0.1513,0.0293,0.0111,-0.1843,-0.2633,-0.2572])
...
plt.savefig('D:/figure改文章12-25/northchinalbm.png',format='png',bbox_inches='tight',dpi=300)